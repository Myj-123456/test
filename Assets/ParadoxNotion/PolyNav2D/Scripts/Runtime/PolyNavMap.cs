...
namespace PolyNav
{
    ...
    public class PolyNavMap : MonoBehaviour
    {
        ...
        /// <returns></returns>
        public bool CheckObstacleIsBlock(Vector2 posA, Vector2 posB)
        {
            if ((posA - posB).magnitude < Mathf.Epsilon)
            {
                return false;
            ...
            for (int j = 0; j < navObstacles.Count; j++)
            {
                var obstacle = navObstacles[j];
                var rad = radiusOffset + obstacle.extraOffset;
                for (var p = 0; p < obstacle.GetPathCount(); p++)
                {
                    var points = obstacle.GetPathPoints(p);
                    var transformed = TransformPoints(ref points, obstacle.transform);
                    var points2 = InflatePolygon(ref transformed, rad);
                    var blockPointCount = 0;//阻挡点
                    for (int i = 0; i < points2.Length; i++)
                    {
                        if (SegmentsCross(posA, posB, points2[i], points2[(i + 1) % points2.Length]))
                        {
                            if (obstacle.isBlock)
                            {
                                ...
                                {
                                    blockPointCount += 1;
                                    Debug.Log("blockPointCount:" + blockPointCount);
                                    if (blockPointCount >= 2)//阻挡两点以上才算是被阻挡了
                                    {
                                        return true;
                                    }
                                }
                                else if (obstacle.blockType == 2)
                                {
                                    return true;
                                }
                            }
        ...
        /// <param name="posB"></param>
        /// <returns></returns>
        public List<Vector2> CheckLOSPosList(Vector2 posA, Vector2 posB)
        {
            ...
            {
                var poly = map.obstaclePolygons[i];
                for (int j = 0; j < poly.points.Length; j++)
                {
                    if (SegmentsCross(posA, posB, poly.points[j], poly.points[(j + 1) % poly.points.Length]))
                    {
                        list.Add(poly.points[j]);
                    }
                }
            }
            return list;
        ...
        public static bool PointInsidePolygon2(Vector2[] polyPoints, Vector2 point)
        {
