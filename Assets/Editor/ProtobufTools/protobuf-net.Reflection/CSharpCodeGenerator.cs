...
namespace ProtoBuf.Reflection
{
    ...
    public class CSharpCodeGenerator : CommonCodeGenerator
    {
        ...
        protected override void WriteEnumHeader(GeneratorContext ctx, EnumDescriptorProto obj, ref object state)
        {
            ...
            //ctx.WriteLine("[System.Serializable]");//修改by aike
            var tw = ctx.Write($@"[ProtoBuf.ProtoContract");
        ...
        protected override void WriteEnumValue(GeneratorContext ctx, EnumValueDescriptorProto obj, ref object state)
        {
            ...
            if (name != obj.Name)
            {
                var tw = ctx.Write($@"[ProtoBuf.ProtoEnum(");
        ...
        protected override void WriteMessageHeader(GeneratorContext ctx, DescriptorProto obj, ref object state)
        {
            ...
            if (obj.ExtensionRanges.Count != 0) tw.Write(" :ProtoBuf.IExtensible");
            tw.WriteLine();
            ctx.WriteLine("{").Indent();
            ...
            {
                ctx.WriteLine($"private ProtoBuf.IExtension {FieldPrefix}extensionData;")
                    .WriteLine($"ProtoBuf.IExtension ProtoBuf.IExtensible.GetExtensionObject(bool createIfMissing)").Indent()
                    .WriteLine($"=> ProtoBuf.Extensible.GetExtensionObject(ref {FieldPrefix}extensionData, createIfMissing);").Outdent().WriteLine();
            }
        }
        ...
        {
            var name = ctx.NameNormalizer.GetName(obj);
            var tw = ctx.Write($@"[ProtoBuf.ProtoMember({obj.Number}");
            if (name != obj.Name)
            {
                tw.Write($@", Name = @""{obj.Name}""");
            }
            ...
            {
                //tw.Write($@", TypeName = ""{GetTypeName2(obj.TypeName)}""");//屏蔽生成TypeName by aike
            }
                ...
                else if (obj.type == FieldDescriptorProto.Type.TypeEnum)
                {
                    ...
                    if (enumType != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(defaultValue))
                        {
                            ...
                            defaultValue = enumType.FullyQualifiedName.Substring(1) + "." + defaultValue;
                        }
                    }
                }
            }
            ...
            {
                ctx.WriteLine($"[System.ComponentModel.DefaultValue({defaultValue})]");
            }
            WriteOptions(ctx, obj.Options);
            if (isRepeated)
            {
                ...
                if (mapMsgType != null)
                {
                    ...
                    bool first = true;
                    //tw = ctx.Write($"[global::ProtoBuf.ProtoMap");//屏蔽生成ProtoMap by aike
                    ...
                    //     first = false;
                    // }
                    //tw.WriteLine(first ? "]" : ")]");////屏蔽生成ProtoMap by aike
            ...
            else if (oneOf != null)
            {
                ...
                ctx.WriteLine($"set {{ {fieldName} = new ProtoBuf.{unionType}({obj.Number}, value); }}")
                    .Outdent().WriteLine("}");

                if (oneOf.IsFirst())
                {
                    ctx.WriteLine().WriteLine($"private ProtoBuf.{unionType} {fieldName};");
                }
            }
        ...
        protected override void WriteExtension(GeneratorContext ctx, FieldDescriptorProto field)
        {
            ...
            else
            {
                ...
                var tw = ctx.WriteLine($"{GetAccess(GetAccess(field))} static {type} Get{name}({@this}{extendee} obj)")
                    .Write($"=> obj == null ? default({type}) : ProtoBuf.Extensible.GetValue<{type}>(obj, {field.Number}");
                if (!string.IsNullOrEmpty(dataFormat))
                {
                    tw.Write($", ProtoBuf.DataFormat.{dataFormat}");
                }
                tw.WriteLine(");");
                ctx.WriteLine();
        ...
        private string FindNameFromCommonAncestor(IType declaring, IType target, NameNormalizer normalizer)
        {
            ...
            while (yStack.Count != 0)
            {
                ...
                {
                    if (sb.Length == 0 && target is FileDescriptorProto) sb.Append("");// sb.Append("global::"); 修改by aike
