...
namespace ProtoBuf.Serializers
{
    sealed class TypeSerializer : IProtoTypeSerializer
    {
        ...
        private readonly System.Collections.Generic.Dictionary<int, int> fieldDict;
        ...
        private readonly Dictionary<int, OneofAttType> oneofAttTypes;
        private readonly Dictionary<string, OneofDecorator> oneofSerializers;
        // ========================================================
        // added by wsh @ 2017-07-01 for net data pool
        private readonly TypeModel model;
        // ========================================================
        public TypeSerializer(TypeModel model, Type forType, int[] fieldNumbers, IProtoSerializer[] serializers, MethodInfo[] baseCtorCallbacks, bool isRootType, bool useConstructor, CallbackSet callbacks, Type constructType, MethodInfo factory,Dictionary<int, OneofAttType> oneofAttTypes)
        {
            ...
            if (this.oneofAttTypes.Count > 0)
            {
                oneofSerializers = new Dictionary<string, OneofDecorator>();
                foreach (OneofAttType att in oneofAttTypes.Values)
                {
                    if (!oneofSerializers.ContainsKey(att.name))
                    {
                        oneofSerializers.Add(att.name, new OneofDecorator(model as RuntimeTypeModel, forType, oneofAttTypes,att.name));
                    }
                }
            ...
            if (fieldNumbers.Length > 300)
            {
                fieldDict = new System.Collections.Generic.Dictionary<int, int>();
                for (int i =0,imax=fieldNumbers.Length; i < imax; i++)
                {
                    fieldDict.Add(fieldNumbers[i], i);
                }
            }
        ...
        public void Write(object value, ProtoWriter dest)
        {
            ...
            if (oneofSerializers != null)
            {
                foreach (IProtoSerializer ser in oneofSerializers.Values)
                {
                    ser.Write(value, dest);
                }
            }
        ...
        public object Read(object value, ProtoReader source)
        {
            ...
            while ((fieldNumber = source.ReadFieldHeader()) > 0)
            {
                ...
                if (fieldDict != null)
                {
                    if (!fieldDict.TryGetValue(fieldNumber, out index)) 
                    {
                        index = -1;
                    }
                }
                ...
                {
                    index = Search(fieldNumbers, lastFieldIndex, fieldNumbers.Length - 1, fieldNumber);
                }

                if (index >= 0)
                {
                    ser = serializers[index];
                ...
                {
                    if (oneofAttTypes!=null && oneofAttTypes.ContainsKey(fieldNumber))
                    {
                        var attType = oneofAttTypes[fieldNumber];
                        ser = oneofSerializers[attType.name];
                        lastFieldIndex = 0;
                        lastFieldNumber = fieldNumber;
                        fieldHandled = true;
                    }
