...
;
        ...
        public override void Write(object value, ProtoWriter dest)
        {
            ...
            // 对于值类型的列表，这里严重GC，判断值类型直接强转解决
            var t = value.GetType();
            if (t == typeof(PbList<uint>))
            {
                var list = value as PbList<uint>;
                foreach (var item in list)
                {
                    lock (ValueTypeCache.mLock)
                    {
                        var subItem = ValueTypeCache.uintCache.GetCacheValue(item);
                        Tail.Write(subItem, dest);

                    }
                }
            }
            else if (t == typeof(PbList<ulong>))
            {
                var list = value as PbList<ulong>;
                foreach (var item in list)
                {
                    lock (ValueTypeCache.mLock)
                    {
                        var subItem = ValueTypeCache.ulongCache.GetCacheValue(item);
                        Tail.Write(subItem, dest);
                    }
                }
            }
            else if (t == typeof(PbList<int>))
            {
                var list = value as PbList<int>;
                foreach (var item in list)
                {
                    lock (ValueTypeCache.mLock)
                    {
                        var subItem = ValueTypeCache.intCache.GetCacheValue(item);
                        Tail.Write(subItem, dest);
                    }
                }
            }
            else if (t == typeof(PbList<long>))
            {
                var list = value as PbList<long>;
                foreach (var item in list)
                {
                    lock (ValueTypeCache.mLock)
                    {
                        var subItem = ValueTypeCache.longCache.GetCacheValue(item);
                        Tail.Write(subItem, dest);
                    }
                }
            }
            else if (t == typeof(PbList<bool>))
            {
                var list = value as PbList<bool>;
                foreach (var item in list)
                {
                    lock (ValueTypeCache.mLock)
                    {
                        var subItem = ValueTypeCache.boolCache.GetCacheValue(item);
                        Tail.Write(subItem, dest);
                    }
                }
            }
            else if (t.IsGenericType && t.GetGenericTypeDefinition() == typeof(PbList<>))
            {
                var l = value as IPbList;
                for (int i = 0, imax = l.Count; i < imax; i++)
                {
                    var subItem = l[i];
