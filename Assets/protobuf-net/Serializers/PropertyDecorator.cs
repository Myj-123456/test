...
{
    delegate  object SetValueFun(object obj);
    sealed class PropertyDecorator : ProtoDecoratorBase
    {
        ...
        private readonly Action<object, object> shadowSetter;
        private readonly SetValueFun writeSetter;
        ...
        public PropertyDecorator(TypeModel model, Type forType, PropertyInfo property, IProtoSerializer tail) : base(tail)
        {
            ...
            var methonInfo = GetShadowSetter(model, property);
            if (methonInfo != null) 
            shadowSetter = (Action<object, object>)methonInfo.CreateDelegate(typeof(Action<object, object>)) ;
            methonInfo = GetWriteSetter(model, property);
            if (methonInfo != null)
                writeSetter = (SetValueFun)methonInfo.CreateDelegate(typeof(SetValueFun));
        }
...
#endif
            MethodInfo method = Helpers.GetStaticMethod(reflectedType, "Set__" + property.Name);
#endif
            if (method == null || !method.IsPublic || method.ReturnType != model.MapType(typeof(void))) return null;
            return method;
        }
        static MethodInfo GetWriteSetter(TypeModel model, PropertyInfo property)
        {
#if WINRT
            MethodInfo method = Helpers.GetInstanceMethod(property.DeclaringType.GetTypeInfo(), "Get" + property.Name, new Type[] { property.PropertyType });
#else
            ...
            Type reflectedType = property.ReflectedType;
#endif
            MethodInfo method = Helpers.GetStaticMethod(reflectedType, "Get__" + property.Name);
#endif
       
            if (method == null || !method.IsPublic || method.ReturnType != model.MapType(typeof(object))) return null;
        ...
        public override void Write(object value, ProtoWriter dest)
        {
            ...
            value = writeSetter!=null? writeSetter(value): property.GetValue(value, null);
