    ...
    public class MetaType : ISerializerProxy
    {
        ...
        private IProtoTypeSerializer BuildSerializer()
        {
            ...
            if (itemType != null)
            {
                ...
                return new TypeSerializer(model, type, new int[] { ProtoBuf.Serializer.ListItemTag }, new IProtoSerializer[] { fakeMember.Serializer }, null, true, true, null, constructType, factory,oneofAttTypes);
            }
            if (surrogate != null)
            {
            ...
            return new TypeSerializer(model, type, fieldNumbers, serializers, arr, baseType == null, UseConstructor, callbacks, constructType, factory, oneofAttTypes);
        }
        ...
        internal void ApplyDefaultBehaviour()
        {
            ...
            for(int i = 0; i < foundList.Length;i++)
            {
                ...
                if((property = member as PropertyInfo) != null)
                {
                    ...
                    isField = false;
                    ApplyDefaultBehaviour_AddMembers(model, family, isEnum, partialMembers, dataMemberOffset, inferTagByName, implicitMode, members, member, ref forced, isPublic, isField, ref effectiveType, oneofAttTypes);
        ...
        private static void ApplyDefaultBehaviour_AddMembers(TypeModel model, AttributeFamily family, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferTagByName, ImplicitFields implicitMode, BasicList members, MemberInfo member, ref bool forced, bool isPublic, bool isField, ref Type effectiveType, Dictionary<int, OneofAttType> oneofAttTypes)
        {
            switch (implicitMode)
            {
            ...
            {
                ProtoMemberAttribute att =member.GetCustomAttribute<ProtoMemberAttribute>();
                if (att != null)
                {
                    if (!string.IsNullOrEmpty( att.OneofName))
                    {
                        OneofAttType attType = new OneofAttType();
                        attType.type = effectiveType;
                        attType.format = att.DataFormat;
                        attType.name = att.OneofName;
                        oneofAttTypes.Add(att.Tag, attType);
                    }
                    else
                    {
                        ProtoMemberAttribute normalizedAttribute = NormalizeProtoMember(model, member, family, att, forced, isEnum, partialMembers, dataMemberOffset, inferTagByName);
        ...
        private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, AttributeFamily family, ProtoMemberAttribute attrib, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName)
        {
            ...
            if (!ignore && !done) // always consider ProtoMember 
            {
                ...
                {
                    fieldNumber = attrib.Tag;
                    dataFormat = attrib.DataFormat;
                    isPacked = attrib.IsPacked;
                    oneofName = attrib.OneofName;
                }
            }
        ...
        private Dictionary<int,OneofAttType> oneofAttTypes = new Dictionary<int, OneofAttType>();
