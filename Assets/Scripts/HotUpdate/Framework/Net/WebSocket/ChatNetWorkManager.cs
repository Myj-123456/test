...
/// 聊天服网络管理器
/// </summary>
public partial class ChatNetWorkManager : Singleton<ChatNetWorkManager>
{
    ...
    {
        Debug.Log("连接聊天服,serverAddress：" + serverAddress);
        Clear();//清除旧状态
    ...
    {
        Debug.LogError("聊天服WebSocket触发OnClose,Code:" + e.Code + " Reason:" + e.Reason);
        OnCloseCall?.Invoke();
    }
    ...
    {
        Debug.LogError("聊天服WebSocket触发OnError:" + e.Message);
        connectFail?.Invoke();
    }
}
