...
namespace ADK
{
    ...
    {
        public static byte[] _byte = new byte[] { 6, 10, 55, 0, 90 };
        private static Transform _adkRoot;

        public enum EditorMode { editing, waitPlaying, playing, quiting }
        public static EditorMode editorMode = EditorMode.playing;


        public static Transform GetADKRoot()
        {

            if (_adkRoot == null)
            {
                GameObject go = GameObject.Find("ADK");
                if (go == null)
                {
                    go = new GameObject("ADK");
                    GameObject.DontDestroyOnLoad(go);
                }
                _adkRoot = go.transform;
            }

            return _adkRoot;
        }
        public static Transform GetADKRootChild(string name)
        {
            Transform adkRoot = GetADKRoot();
            Transform child = adkRoot.Find(name);
            if (child == null)
            {
                GameObject go = new GameObject(name);
                child = go.transform;
                child.parent = adkRoot;
            }

            return child;
        }
        public static T GetADKRootComponent<T>(string name) where T : Component
        {
            Transform go = GetADKRootChild(name);
        ...
        public static void SetPosByMouse(Transform transform, int upOrDown = 0)
        {
            ...
            if (parent != null)
            {
                Vector2 pos = ADKTool.MouseToLocalPoint(rt.parent);
...
#elif !UNITY_EDITOR && UNITY_WEBGL && WEIXINMINIGAME//微信小游戏
            WeChatWASM.ExitMiniProgramOption onExitMiniProgramOption = new WeChatWASM.ExitMiniProgramOption();
            WeChatWASM.WX.ExitMiniProgram(onExitMiniProgramOption);
        ...
        public static void RestartApp()
        {
            ...
            UnityEditor.EditorApplication.isPlaying = false;
#elif !UNITY_EDITOR && UNITY_WEBGL && WEIXINMINIGAME//微信小游戏
                      WeChatWASM.RestartMiniProgramOption onRestartMiniProgram = new WeChatWASM.RestartMiniProgramOption();
                      WeChatWASM.WX.RestartMiniProgram(onRestartMiniProgram);
#elif !UNITY_EDITOR && UNITY_ANDROID//安卓端
                       RestartAndroid();
#elif !UNITY_EDITOR && UNITY_IOS//ios没法重启 修改为关闭
                        Application.Quit();
#endif
        }
        ...
        /// <returns></returns>
        public static bool AreArraysEqual(uint[] array1, uint[] array2)
        {
            // 如果数组引用相同，直接返回 true
            if (array1 == array2) return true;

            // 如果有一个数组为 null 或长度不同，返回 false
