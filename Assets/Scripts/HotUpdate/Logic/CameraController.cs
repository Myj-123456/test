...
public class CameraController : MonoBehaviour
{
    ...
    public void SetMap(Vector2 mapSize)
    {
        if (mainCamera == null)
    ...
    void Update()
    {
        ...
        if (GuideModel.Instance.IsPrequelPlotGuiding || GuideModel.Instance.IsGuiding) return;
        if (SceneManager.Instance.IsEditing) return;
    ...
    private void MoveZoomEditor()
    {
        ...
        if (scroll != 0)
        {
            ...
            {
                SceneManager.Instance.HideHarvest();
            }
            SceneManager.Instance.HideOneKeyWatering();
            GuideController.Instance.CancelWeakGuide();
        }
    ...
    private void OrgPointReleaseTouch()
    {
        ...
        if (hit.collider != null)//大部分场景物都不需要响应 除了地板
        {
            var decoration = hit.collider.gameObject.GetComponent<Decoration>();
            if (decoration != null && (decoration.decorationsType != DecorationsType.Counter || decoration.decorationsType != DecorationsType.FlowerStand))
            {
                isReturn = false;
            }
            else
    ...
    {
        if (SceneManager.Instance.isLongPress) return;
