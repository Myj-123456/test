...
public class RechargeController : BaseController<RechargeController>
{
    ...
    {
        if (data.payType == 40)//payTypeΪ40ΪIOS
        {
            if (!string.IsNullOrEmpty(data.appAccountToken))//iOS在官方支付需要传appAccountToken，payType为40时才会返回，否则返回空字符串
            {
    ...
    /// 发货接口
    /// </summary>
    /// <param name="payConfType">支付配置类型id  1:ft_diamond_gift_pack 2:ft_diamond_value 3:ft_game_pay </param>
    /// <param name="payDefId">支付定义id</param>
    /// <param name="osType">操作系统类型 1:web 2:安卓 3:ios</param>
    /// <param name="payType">支付类型 1:移动端支付 2:移动端jsapi支付 3:移动端NATIVE支付 11:PC端支付 12:PC端H5支付 30:web支付</param>
    /// <param name="platformType">支付平台类型 1:web 2:移动端 3:PC端 4:app</param>
    public void ReqPlaceOrder(uint payConfType, uint payDefId)
    {
        ...
        if (LoginHelper.GetPlatform() == "app")//app支付(web支付为移动端支付)
        {
            c_MSG_PLACE_ORDER.payType = 30;
            c_MSG_PLACE_ORDER.platformType = 4;
        }
        else if (LoginHelper.GetPlatform() == "wm")//移动端支付(web支付为移动端支付)
        {
            c_MSG_PLACE_ORDER.payType = 30;
            c_MSG_PLACE_ORDER.platformType = 2;
        }
        else if (LoginHelper.GetPlatform() == "dev")//开发版本web支付(web支付为移动端支付)
        {
            c_MSG_PLACE_ORDER.payType = 30;
        ...
        if (responseResult.code == 0)
        {
            ...
            else if (data.payConfType == 2)
            {
                ...
                if (RechargeModel.Instance.diamondValueHome.ContainsKey((int)data.payDefId))
                {
                    ...
                    {
                        case E_DIAMOND_VALUE_TYPE.DAILY://每日循环礼包

                            break;
                        case E_DIAMOND_VALUE_TYPE.NORMAL://普通礼包
                            EventManager.Instance.DispatchEvent(RechargeEvent.Normal);
                            break;
                        case E_DIAMOND_VALUE_TYPE.VIP://Vip奖励

                            EventManager.Instance.DispatchEvent(RechargeEvent.VipPay);
                            break;
                        case E_DIAMOND_VALUE_TYPE.VIDEO_PRIVILEGE://视频奖励


                            EventManager.Instance.DispatchEvent(RechargeEvent.VideoPay);
                            break;

                        case E_DIAMOND_VALUE_TYPE.CONTRACT://合同奖励
                        case E_DIAMOND_VALUE_TYPE.CONTRACT_SUPER://超级合同奖励
                        case E_DIAMOND_VALUE_TYPE.BUY_CONTRACT_LEVEL://购买合同等级
    ...
    public void RechargeFree(S_MSG_RECHARGE_FREE data)
    {
        var dropList = ItemModel.Instance.GetDropData(data.items);
        DropManager.ShowDrop(dropList);
        //支付配置类型id  1:ft_diamond_gift_pack 2:ft_diamond_value 3:ft_game_pay
