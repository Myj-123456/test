...
public class RechargeModel : Singleton<RechargeModel>
{
    ...
    public List<I_GIFTPACK_VO> giftPackList;//礼包列表
    public List<uint> giftHaveBuy;//已购买的礼包id

    public Dictionary<uint, uint> haveDiamondValue; //已购买的钻石值

    public Dictionary<int, Dictionary<string, int>> firstRechargeReward = new Dictionary<int, Dictionary<string, int>>();

    public uint[] rechargeRewards;//获取的钻石值

    public uint[] firstRechargeRewards;//首次充值获取的钻石值

    public uint rechargeAmount;//已充值的钻石值

    public GAME_PAY_VO gamePay;//game_pay 游戏支付配置

    public uint[] haveGiftPack;//ft_gift_pack已购买的礼包id

    public Dictionary<uint, uint> haveMall;//ft_mall已购买的商品id 商品id 购买次数

    public uint firstRechargeTime;//首次充值的时间
...
public enum E_DIAMOND_VALUE_TYPE
{
    ...
    STEP = 53,//等级
    CONTRACT = 61,//合同
    CONTRACT_SUPER = 62,//超级合同
    BUY_CONTRACT_LEVEL = 63,//购买合同等级
}
