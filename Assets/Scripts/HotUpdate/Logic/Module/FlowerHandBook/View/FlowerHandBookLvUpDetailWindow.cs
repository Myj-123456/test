...
public class FlowerHandBookLvUpDetailWindow : BaseWindow
{
   ...
   public FlowerHandBookLvUpDetailWindow()
    ...
    private void RenderItem(int index,GObject cell)
    {
        ...
        item.type.selectedIndex = index % 2;
        ...
        if (index != 0)
        {
            item.timeLab.color = _featureList[index].Interval != _featureList[index - 1].Interval ? StringUtil.HexToColor("#12a702") : StringUtil.HexToColor("#335660");
            item.seedLab.color = _featureList[index].SeedProp != _featureList[index - 1].SeedProp ? StringUtil.HexToColor("#12a702") : StringUtil.HexToColor("#335660");
            item.flowerLab.color = _featureList[index].CropCount != _featureList[index - 1].CropCount ? StringUtil.HexToColor("#12a702") : StringUtil.HexToColor("#335660");
            item.countLab.color = _featureList[index].Frequency != _featureList[index - 1].Frequency ? StringUtil.HexToColor("#12a702") : StringUtil.HexToColor("#335660");
        }
        else
        {
            item.timeLab.color =  StringUtil.HexToColor("#335660");
            item.seedLab.color = StringUtil.HexToColor("#335660");
            item.flowerLab.color =  StringUtil.HexToColor("#335660");
            item.countLab.color =  StringUtil.HexToColor("#335660");
        }
    }
